...
function convertBounds(
  { left: newLeft, top: newTop, right: newRight, bottom: newBottom }: Bounds,
  { left, top, right, bottom, angle }: Bounds
) {
...
export default class ResizeView extends RenderView {
  ...
  setStyle({
    ...
    handlerFillColor = DEFAULT_COLOR,
  ...
  setMode(mousePoint: Point) {
    ...
    if (dist(newMousePoint, point(left, top)) <= this.handlerSize) {
      ...
      dist(newMousePoint, point(centerX, bottom)) <= this.handlerSize
    ) {
      ...
      dist(newMousePoint, point(centerX, top - 50)) <= this.handlerSize
    ) {
  ...
  changeMoveBounds(
    ...
    this.setBounds({
      left: newBoundsX,
      top: newBoundsY,
      right: newBoundsX + right - left,
      bottom: newBoundsY + bottom - top,
      angle
    });
  }
  ...
  changeAngle({ x, y }: Point) {
    ...
    this.setBounds({
      ...
      angle:
        x > centerX
          ? 90 + (Math.atan2(y - centerY, x - centerX) * 180) / Math.PI
          : 270 + (Math.atan2(centerY - y, centerX - x) * 180) / Math.PI
    });
  }
}
